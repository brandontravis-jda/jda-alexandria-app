    ...
    <div className="max-w-3xl">
      ...
      <div className="flex flex-col gap-5">
        <div>
          ...
          <textarea value={data.platform_intro} onChange={(e) => setData({ ...data, platform_intro: e.target.value })} rows={4} readOnly={!canEdit}
        ...
        <div>
          ...
          <textarea value={data.feedback_prompt} onChange={(e) => setData({ ...data, feedback_prompt: e.target.value })} rows={4} readOnly={!canEdit}
            className="text-sm px-3 py-1.5 rounded-md w-full" style={{ background: "var(--color-jda-bg-surface)", color: "var(--color-jda-cream)", border: "1px solid var(--color-jda-border)", resize: "vertical" }} />
        </div>
            ...
            {data.canonical_entry_prompts.map((entry, i) => (
              <div key={i} className="p-3 rounded-lg" style={{ background: "var(--color-jda-bg-surface)", border: "1px solid var(--color-jda-border)" }}>
                <div className="flex gap-2 mb-2">
                  <input type="text" placeholder="Label" value={entry.label} readOnly={!canEdit} onChange={(e) => {
                ...
                <textarea placeholder="Prompt" value={entry.prompt} readOnly={!canEdit} onChange={(e) => {
                  const updated = [...data.canonical_entry_prompts];
        ...
        <div>
          ...
          <div className="flex flex-col gap-3">
            {data.example_prompts.map((entry, i) => (
              <div key={i} className="p-3 rounded-lg" style={{ background: "var(--color-jda-bg-surface)", border: "1px solid var(--color-jda-border)" }}>
                <div className="flex gap-2 mb-2">
                  <input type="text" placeholder="Use Case" value={entry.useCase} readOnly={!canEdit} onChange={(e) => {
