  ...
  return (
    <footer className="border-t border-brand-border bg-brand-surface">
      <Container>
        {data?.columns && data.columns.length > 0 && (
          <div className="grid gap-8 py-12 sm:grid-cols-2 lg:grid-cols-4">
            {data.columns.map((column, i) => (
              <div key={i}>
                {column.title && (
                  <h3 className="mb-4 font-display text-sm font-semibold uppercase tracking-wider text-brand-text-heading">
                    {column.title}
                  </h3>
                )}
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-brand-border py-6 sm:flex-row">
          <p className="text-sm text-brand-muted">
            {data?.copyrightText
              ? data.copyrightText.replace("{year}", String(year))
              : `\u00A9 ${year} JDA Worldwide. All rights reserved.`}
          </p>
...
function SocialIcon({ social }: { social: FooterSocialLink }) {
  return (
    ...
    >
      {socialSvgs[social.platform] ?? (
        <span className="text-xs font-medium uppercase">{social.platform}</span>
      )}
    </a>
  );
}
